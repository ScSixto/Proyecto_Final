...
public class TesterMetodos {
	...
	public static String formatDouble(double num) {
		...
		while (i >= 0
				&& parcialFormatedDouble.charAt(i) != doubleFormat
						.getDecimalFormatSymbols().getDecimalSeparator()) {
			formatedDouble = parcialFormatedDouble.charAt(i) + formatedDouble;
			i--;
		}
		formatedDouble = formatInteger(Long.parseLong(parcialFormatedDouble
				.substring(0, i)))
				+ doubleFormat.getDecimalFormatSymbols().getDecimalSeparator()
				+ formatedDouble;
	...
	public static String formatInteger(long num) {
		...
		for (int i = parcialFormatedInteger.length() - 1; i >= 0; i--) {
			formatedInteger = getNumericSeparator(thousandCount)
					+ formatedInteger;
			for (int j = 0; j <= i && j < 3; j++) {
				formatedInteger = parcialFormatedInteger.charAt(i - j)
						+ formatedInteger;
			}
			i -= 2;
			thousandCount++;
