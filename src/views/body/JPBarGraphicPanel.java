...
public class JPBarGraphicPanel extends JPanel {
	...
	public JPBarGraphicPanel(ActionListener actionListener,
		...
		// this.setMaximumSize(new Dimension((int)
		// (ConstantsGUI.MAXIMUM_SCREEN_WIDTH * 4 / 5),
		// ConstantsGUI.MAX_PIXEL_COL_HEIGHT_VALUE +
	...
	private void addPanelList() {
		...
		.addGroup(
				layout.createSequentialGroup()
						.addComponent(this.yAxisPanel)
						.addGroup(
								layout.createParallelGroup()
										.addComponent(this.graphicBarPanel)
										.addComponent(this.xAxisPanel))
		...
		layout.createParallelGroup()
				...
				.addGroup(
						layout.createSequentialGroup()
								.addComponent(graphicBarPanel)
								.addComponent(xAxisPanel))
				.addComponent(this.panelInformation));
	}
	...
	private void setAxis(HashMap<String, Double> valueList) {
		...
		this.xAxisPanel = new JPXAxisBarGraphic(values.size(),
				yAxisPanel.getWidth());
	}

	private void setGraphicBarPanel(HashMap<String, Double> valueList) {
		this.setGraphicBarPanelFormat();
		this.graphicBarPanel
				.setMaximumSize(new Dimension(
						(ConstantsGUI.MAX_PIXEL_COL_WIDTH_VALUE * valueList
								.values().size())
								+ (valueList.values().size() * ConstantsGUI.COL_SEPARATION),
						ConstantsGUI.MAX_PIXEL_COL_HEIGHT_VALUE
								+ ConstantsGUI.AXIS_LINE_WIDTH));
		HashMap<String, Color> columnInformation = new HashMap<>();
		Iterator<Entry<String, Double>> it = UtilView
				.getPercentValues(valueList).entrySet().iterator();
		Color color;
		while (it.hasNext()) {
			Entry<String, Double> column = it.next();
			color = UtilView.getRandomColor();
			columnInformation.put(column.getKey(), color);
			this.graphicBarPanel.add(new JLColumnLabel(UtilView
					.formatDouble(valueList.get(column.getKey())), column
					.getValue(), color));
		}
	...
	public void setGraphicBarPanelFormat() {
		...
		this.graphicBarPanel.setBorder(BorderFactory.createEmptyBorder(0, 0, 0,
				0));
