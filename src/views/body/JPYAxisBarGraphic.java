...
public class JPYAxisBarGraphic extends JPanel {
	...
	public JPYAxisBarGraphic(ArrayList<Double> values) {
		...
		this.setMaximumSize(new Dimension(this.valuesPanel.getWidth()
				+ ConstantsGUI.AXIS_LINE_WIDTH, axisHeight));
	}
	...
	private void initComponents(ArrayList<Double> values) {
		...
		for (int i = 0; i < ConstantsGUI.Y_AXIS_VALUE_QUANTITY; i++) {
			JLabel valueLabel = new JLabel(
					UtilView.getRoundedValue(this.maxValue - this.maxValue * i
							/ ConstantsGUI.Y_AXIS_VALUE_QUANTITY));
			...
			valueLabel.setBorder(BorderFactory.createEmptyBorder(0, 0,
					ConstantsGUI.MAX_PIXEL_COL_HEIGHT_VALUE
							/ (ConstantsGUI.Y_AXIS_VALUE_QUANTITY)
							- ConstantsGUI.DATA_LABEL_FONT.getSize() - 1, 3));
	...
	public void setValuesPanel() {
		...
		BoxLayout boxLayout = new BoxLayout(this.valuesPanel,
				BoxLayout.PAGE_AXIS);
	...
	public void paint(final Graphics g) {
		...
		g.fillRect(this.valuesPanel.getWidth() - ConstantsGUI.AXIS_LINE_WIDTH,
				0, ConstantsGUI.AXIS_LINE_WIDTH, this.axisHeight);
		// g.getFont().deriveFont(ConstantsGUI.DATA_LABEL_FONT.getStyle(),
		// ConstantsGUI.DATA_LABEL_FONT.getSize());
		// int y = ConstantsGUI.DATA_LABEL_FONT.getSize()+1;
		// for (int i = 0; i < ConstantsGUI.Y_AXIS_VALUE_QUANTITY; i++) {
		// g.drawString(UtilView.getRoundedValue(this.maxValue - this.maxValue *
		// i/ConstantsGUI.Y_AXIS_VALUE_QUANTITY), 0, y);
		// y += ConstantsGUI.MAX_PIXEL_COL_HEIGHT_VALUE /
		// ConstantsGUI.Y_AXIS_VALUE_QUANTITY;
		// }
		// g.drawString("colName",0, (int)
		// ((ConstantsGUI.MAX_PIXEL_COL_HEIGHT_VALUE *
		// ConstantsGUI.MAX_PIXEL_COL_HEIGHT_VALUE) - colHeight));
	}
}